...
{  
  ...
  // wait to send data for other device to give it information
  if (mySerial.available() > 0){
    // check that it is not bad data
    checksum = mySerial.read();
    Serial.println("Recieved Checksum!");
    Serial.println(int(checksum));

    if (checksum == 65){  // capital case "a"
      // this is a valid string we are sending from the other device
      // send back "a" or 97
      mySerial.write(byte(97));

      Serial.println("Sent 2nd Checksum!");
      
      // wait for acknowledgement then send all data
      char checksum2 = mySerial.read();
      Serial.println("Recieved 2nd Checksum!");
      Serial.println("Sending data");

      if (checksum2 == 66) { // "capital B"
        // NOTE: getChannelValue is relative to the configured startaddress
        ...
        // wait 100 ms before sending more data
        delay(100);
      }
    }  
  }
